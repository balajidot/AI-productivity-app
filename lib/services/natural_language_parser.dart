...
class NaturalLanguageParser {
  static ParsedTaskData parse(String input) {
    ...
    // --- Time Detection ---
    // "at 10am", "at 2pm", "at 10:30am", "at 14:00"
    ...
    // --- Category Detection ---
    final categoryMatch = RegExp(r'[@#](work|personal|health|study|finance|shopping)', caseSensitive: false).firstMatch(text);
    ...
    // Clean up: remove extra spaces, "at" left alone, etc.
    text = text.replaceAll(RegExp(r'\s+'), ' ').trim();
    // Remove trailing "at" if alone
    text = text.replaceAll(RegExp(r'\bat\b\s*$', caseSensitive: false), '').trim();
    // Remove leading "at"
    text = text.replaceAll(RegExp(r'^\bat\b\s*', caseSensitive: false), '').trim();

    return ParsedTaskData(
      cleanTitle: text.isEmpty ? input.trim() : text,
      date: parsedDate,
      time: parsedTime,
  ...
  static List<String> getSuggestions(String input) {
    ...
    if (lower.isNotEmpty && !lower.contains('today') && !lower.contains('tomorrow')) {
      suggestions.add('$input today');
      suggestions.add('$input tomorrow');
    }
    if (lower.isNotEmpty && !RegExp(r'\d{1,2}\s*(am|pm)', caseSensitive: false).hasMatch(lower)) {
      suggestions.add('$input at 9am');
      suggestions.add('$input at 2pm');
    }
    if (lower.isNotEmpty && !lower.contains('daily') && !lower.contains('everyday')) {
  ...
  static String _removeWord(String text, String word) {
    return text.replaceAll(RegExp('\\b${RegExp.escape(word)}\\b', caseSensitive: false), '');
  }
